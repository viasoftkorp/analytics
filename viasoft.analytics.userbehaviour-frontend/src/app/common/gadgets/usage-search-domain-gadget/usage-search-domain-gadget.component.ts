...
@Component({
  ...
  templateUrl: './usage-search-domain-gadget.component.html',
  styleUrls: ['./usage-search-domain-gadget.component.scss'],
})
export class UsageSearchDomainGadgetComponent extends UsageSearchBaseGadget implements OnInit, OnDestroy {
  constructor(
    injector: Injector,
  ...
  ngOnInit() {
    this.subs.add("domainRender", this.usageSearchDomainGadgetService.renderObservable.subscribe(
      () => {
  ...
  getCategories(input: FilterByGroupingOutput) {
    return input.values.map(v => {
      const path = `userBehaviour.domain.domains.${v.domainName.toLocaleLowerCase()}`;
      const translatedDomain =
      this.translateService.instant(path);
      if (translatedDomain === path) {
        return v.domainName;
      } else {
  ...
  get currentGroupingLabel(): any {
    return 'dashBoard.usageSearch.groupings.domain.label|dateInterval:' + this.dateTimeIntervalOptions.filter(e => e.value === this.appliedDateInterval)[0].name;
  }
